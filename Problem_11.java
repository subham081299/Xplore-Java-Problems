...
public class Problem_11 {
    public static void main(String[] args) {
        ...
        sc.close();
        int ans1 = findAvgBudgetByDirector(movies,director);
        if (ans1 != 0)
        {
        ...
        int ans2 = getMovieByRatingBudget(movies,rating,budget);
        if (ans2 != 0)
        {
    ...
    private static int getMovieByRatingBudget(Movie[] movies, int rating, int budget) {
        for (int i = 0 ; i <4 ; i++)
        {
            if (movies[i].getRating() == rating && movies[i].getBudget() == budget)return movies[i].getMovieId();
        }return 0;
    }
