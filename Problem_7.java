...
public class Problem_7 {
    public static void main(String[] args) {
        ...
        for (int i = 0 ; i<4 ; i++)
            institutions[i] = new Institution(sc.nextInt(), sc.next(), sc.next(),sc.nextInt(), sc.next());
        String location = sc.next();
        String institution_name = sc.next();
        int ans1 = findNumClearancedByLoc(institutions,location);
        Institution ans2 = updateInstitutionGrade(institutions,institution_name);
        sc.close();
        if (ans1>0) System.out.println(ans1);
        else System.out.println("There are no cleared students in this particular location");
        if (ans2 != null)
        {
            System.out.println(institution_name + "::" + ans2.getGrade());
    ...
    public static int findNumClearancedByLoc(Institution[] instArray, String location)
    {
        ...
        for (int i = 0 ; i<4 ; i++)
        {
            ...
            {
                sum+=instArray[i].getNoOfStudentsCleared();
            }
        }
        return sum;
    }
    public static Institution updateInstitutionGrade(Institution[] instArray, String instName)
    {
        ...
        for (int i = 0 ; i<4 ;i++)
        {
            ...
            {
                int marks = (Integer.parseInt(instArray[i].getNoOfStudentsPlaced())*100)/instArray[i].getNoOfStudentsCleared();
                if (marks>=80)instArray[i].setGrade("A");
...
class Institution{
     private int institutionId , noOfStudentsCleared;
     private String institutionName , noOfStudentsPlaced , location , grade;
